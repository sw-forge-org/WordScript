...
- **Overlay-Diagnose-Infrastruktur permanent, debug-only (2026-07-20):** Vier neue Rust-Commands (`overlay_open_devtools`, `append_diag_log`, `read_diag_log`, `clear_diag_log` in `src-tauri/src/lib.rs`) + DEV-only `OverlayDiagPanel`-Komponente in Settings (Sidebar-Area "Overlay Diag", production-stripped via `import.meta.env.DEV`). Das Panel pollt `/tmp/kilo/overlay-diag.log` alle 500ms, bietet Auto-Scroll, "Open Overlay DevTools"- und "Clear Diag Log"-Buttons. Frontend (`src/windows/OverlayWindow.tsx`) schreibt `[ov-tap]`/`[ov-render]`/`[ov-sched]`/`[ov-repaint]`/`[ov-dom]`/`[ov-reveal]`-Logs via `diagLog()` Helper (console.warn + invoke). `overlay_open_devtools` ist cfg-gated auf `debug_assertions || feature = "devtools"` (Tauri v2 constraint). Investition in zukuenftige Overlay-Diagnose ohne Ad-hoc-Commands bauen zu muessen. Plan `.kilo/plans/1784433288646-overlay-ghosting-compositor-diagnosis.md`. Handoff `docs/handoffs/OVERLAY_MODE_CYCLING_GHOSTING_ACCEPTED.md`.
...
- **Capture-Stream-Rebuild nach transistentem StreamError (2026-07-18):** Wenn der cpal-StreamError-Callback waehrend einer aktiven Aufnahme feuert (PipeWire/PulseAudio-Suspend/Resume, Compositor-Restart, Device-Reenumerate), startet WordScript jetzt **genau einen** Rebuild-Versuch mit `cpal::default_host()`/`default_input_device()` und frischem `stream_error`-AtomicBool, bevor der bestehende Recovery-Flow (Error-Pill → Processing-Preview mit Copy) greift. `ActiveCapture.rebuild_attempted` erlaubt genau einen Versuch pro Session (verhindert Retry-Loops); `SharedCaptureData.rebuild_in_progress` pausiert Sample-Writes und Level-Events waehrend des Rebuilds. **Matching-Gate:** Rebuild nur bei identischer Sample-Rate/Channels/Format wie der alte Stream — sonst `RebuildOutcome::Failed` und Fall-through zum bestehenden Recovery-Flow (Sample-Rate-Wechsel mitten in der Aufnahme wuerde heterogene Samples erzeugen). `monitor_native_capture` liefert neue `NativeCaptureMonitorState::RebuildEligible`-Variante; `spawn_native_capture_monitor` in `lib.rs` ruft `rebuild_stream_after_error` **ausserhalb** des State-Locks aus (vermeidet Deadlock mit blockierendem `build_stream`). Log-Zeile `[WordScript] Native capture stream rebuilt …` im persistenten Runtime-Log; `capture_rebuilt`-Event an Frontend (silent MVP, keine eigene Overlay-Visualisierung). 3 neue Unit-Tests (`does_not_stop_while_rebuild_is_in_progress`, `rollback_rebuild_pause_*`); `Rebuilt`-Pfad manuell via `pactl suspend-source <default-source> 1` waehrend aktiver Aufnahme verifizierbar. Praeventiv ergaenzt: WirePlumber-Keep-Alive-Doku gegen PipeWire-Auto-Suspend idle Input-Sources (siehe `docs/PLATFORMS.md`). Branch `fix/capture-stream-rebuild`. Plan `.kilo/plans/1784377955445-capture-stream-error-diagnosis.md` Task 4 + §0c.
...
- **Overlay-Ghosting-Workaround revertiert (2026-07-20):** Der Vorgänger-Plan (`1784429726777`) hatte das Mode-Cycling-Ghosting von 27px auf ~1px residual reduziert, aber nur durch einen Workaround (feste ModeChip-Breite `min-width: 80px` + `justify-content: center` + `key={state.mode}` auf ModeChip + `.pill--open` transform entfernt). Dieser Workaround ist revertiert: die Pill hat ihre natürliche `max-content`-Breite zurück, die `key={state.mode}`-Remounts sind entfernt, `.pill--open { transform: translateZ(0) scale(1); }` ist wiederhergestellt. **Live verifiziert:** ohne den Workaround funktioniert das Mode-Cycling in der Praxis ohne State-Überlagerung. Zwei Rest-Artefakte (kleiner schwarzer Blitz, kaum sichtbarer horizontaler Strich) sind bewusst akzeptiert — der Stand bleibt auf unbestimmte Zeit so. Behalten: A2 `queueMicrotask` (d902790, schwarzer Blitz) + A1.i `zoom: 0.87` (200829f, neutral). Phase 2/3 (A/B/C Compositor-Live-Diagnose) übersprungen. Revertiert: commits `796ad59` + `89471c4`. Plan `.kilo/plans/1784433288646-overlay-ghosting-compositor-diagnosis.md`. Handoff `docs/handoffs/OVERLAY_MODE_CYCLING_GHOSTING_ACCEPTED.md`.
