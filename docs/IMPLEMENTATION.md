...
| **Home activity** | 2026-08-16 | **Stage A closed, A1–A11**, four of them owner-driven correction passes. Stage B is three things other tracks owe. **Stage C closed 2026-08-17** — twelve rows from an owner brief, eleven landed and one withdrawn, two of them reversing a standing ADR. **Stage D closed 2026-08-18** — one owner reading of the running block, four rows, all frontend: the time-saved foot was claiming a four-week window on a three-day record, and the same tile had no unit ceiling, so the span now ramps and the figure climbs minutes to hours to days (ADR 0233); each metric opens its own view of the block, day to year, with a grain offered only once the ledger reaches three buckets of it (ADR 0235); the week starts on Monday everywhere. **Stage E closed 2026-08-18** — the same evening, five rows from reading Stage D's views against 447 real dictations: turnaround is five bands rather than a 25 ms histogram with eleven empty columns, and under them a list of which model and vendor the wait belongs to; the record now stores the language it was counted as, because the naming call answered for 74 of 75 runs and nothing wrote the answer down, so every ledger rebuild re-measured with the offline detector and dropped the short ones; the decimal point stopped touching the digit before it; the view dots leave the screen inside a metric instead of merely going inert (ADR 0236). **The finding worth knowing outside this track**: five of Stage D's defects were green in Vitest and visible only on a rendered page, one of them a chart drawn at 16 px because `.ws-win svg` is an unlayered presentation hint that beats the stylesheet — the trap the calendar's own file documents and solves with an inline style — and Stage E is the same rule again with the REAL config, ledger and history behind a stubbed `__TAURI_INTERNALS__`, which is the only way one recogniser showing up under two vendors was ever going to be visible. **Stage B is what is left and it is three things other tracks owe** | [`tracks/home-activity.md`](tracks/home-activity.md) | the same file — it is both |
| **Insert delivery** | 2026-08-18 | **Closed 2026-08-18: steps 1-7 done, 8 deliberately deferred.** Opened from one owner report that insert at cursor does nothing here. The measured finding is that the hybrid XWayland lane has exactly one paste driver, XTEST, which exits 0 whether or not the keystroke is delivered — nine consecutive runs recorded `pasted: true` while inserting nothing, because a native Wayland window held the focus and the X server had no focused client at all. Step 1 makes the failure honest (ADR 0227); the rest is the missing lane. **Leg 1 closed 2026-08-18** — all four items, plus one the measurement opened. Its finding is worth knowing outside this track: the click that ended a dictation was **not** a synthetic key release (44 of 44 measured releases came from the device stream), it was WebKitGTK's context menu holding the keyboard over an overlay that had been hidden rather than closed, so the native menu is gone from every window (ADR 0230). The two delivery switches are built (ADR 0231), and a ceiling-stopped dictation now says so on its record. One residual is recorded rather than fixed: a single double reveal at the first surface change after app start, with the likely cause named and the one-field instrument that would prove it. **Leg 1 part 2 closed 2026-08-18** — Leg 1's two leftovers plus four owner reports from the shipped build, and its lesson is the shape of the items rather than any one: six were a thing written to hold a fact that could not hold it (a `cfg` gate on an undeclared feature, a test with no `#[test]`, an attribute counted twice, a command whose only field has no reader, a resolver that dropped the fields it was read for, an event loop that spun silently on a dead X connection), and four were found by reading the file the report pointed at rather than by reproducing it. **The one worth knowing outside this track**: the ADR 0231 delivery switches shipped INOPERABLE while their tests stayed green, because the tests assert on the write and the defect was on the read — `cloneTextProfileWorkMode` enumerated six of ten fields, so it was simultaneously the permanently-false `checked` and the base of every write that erased the other switch. The one-field reveal instrument Leg 1 asked for also went in and **refuted the hypothesis it was built for on the first app start**: same surface twice, 108 ms apart, one flush — so the settle window is not in play and the second reveal came from a native route. Two reports are left with a reading key rather than a fix: a hold that turns into a toggle (one lost key release, three causes the log already separates) and a 3-minute ceiling that is in no log at all (`capture_stop_reason` null on all 369 records; the delivery-mode correlation is a profile correlation). The one item that leg left to the owner is settled: `configure_native_capture` is removed with its field, its request type and the frontend's call (ADR 0232), and the active profile's capture block stays the single source of both ceilings. **Leg 2 built 2026-08-18, steps 4-8 done or deliberately deferred, and the measurement it rested on is in.** The blocker is gone: `core/portal_session.rs` holds a RemoteDesktop session open on a persistent `ashpd` connection on its own thread, and `NativeInsertDriver::RemoteDesktopPortal` sends Ctrl+V through `NotifyKeyboardKeysym` on it — the first Linux paste whose delivery is a call with a result, so on that lane the clipboard restore withheld since ADR 0229 runs again. The focus probe picks exactly one driver per run and never stacks two. **The two findings worth knowing outside this track are not the driver**: the portal path had been closed on the reporting machine the whole time, twice over and in silence — `detect_compositor()` searched for `"plasma"` in variables that read `KDE`, so a Plasma 6 desktop was classified `Other` and the early return that closed the path carried no log line; and the interface probe grepped `busctl --user list`, which lists bus *names*, for an *interface* name that can only be a bus name, so `has_remote_desktop_portal` was false on every machine while `get-property ... RemoteDesktop version` answers `u 2`. 6539 runtime-log lines contained not one portal line. Both are the same shape: **a probe answering "no" for a reason unrelated to the question**, and a consumer treating it as a fact about the machine. The grant flow is the owner's: one button in Delivery & Insert, no dictation ever raises the dialog, a refusal is remembered rather than re-asked (ADR 0228 now Accepted, ADR 0234). **Measured 2026-08-18, and the driver holds**: the grant was pressed once, two dictations delivered through the portal into native Wayland windows in 9 ms and 2 ms, and twelve app restarts with nothing pressed restored the session in 7-20 ms with no dialog and an unrotated token — KWin honours `ExplicitlyRevoked`, so ADR 0228 stands as written rather than needing revision. A second review pass then found the portal's timeouts being paid by the main thread: `native_insertion_status` was a synchronous command asking the one-at-a-time portal thread twice, so a status read could freeze the app for 1.5 s while the permission dialog was up, `restore_last_transcript` ran a whole insert there, and app start spent 120–215 ms spawning probe processes before the first frame. **Both lanes are now measured from real dictations**, which closes the track: the portal into native Wayland windows (9 ms, 2 ms) and `xdotool` into an XWayland window (31 ms, `clipboard_restore: scheduled` — a restore only a confirmable delivery allows). The first attempt at that last one measured the wrong thing and is worth knowing: KDE editors are single-instance, so `QT_QPA_PLATFORM=xcb kate <file>` hands the file to the running **Wayland** instance and the variable never reaches a process — four dictations went into a window that looked like an XWayland target and was not, and the runtime correctly chose the portal for all four. `--new` is what forces a real X client. `ConnectToEIS`/libei was considered and deferred, with the reason recorded | [`tracks/insert-delivery.md`](tracks/insert-delivery.md) | Leg 2's spent brief: [`archive/insert-delivery-kickoff.md`](archive/insert-delivery-kickoff.md) — the portal driver. Leg 1's records are [`tracks/open-fixes-leg1.md`](tracks/open-fixes-leg1.md) and [`tracks/open-fixes-leg1-part2.md`](tracks/open-fixes-leg1-part2.md) |
...
0189–0196 for Stage C, and holds **0233**, **0235** and **0236** for Stages D
and E; 0234 was lost to the insert-delivery track mid-session and renumbered.
