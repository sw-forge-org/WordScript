...
| **Home activity** | 2026-08-16 | **Stage A closed, A1–A11**, four of them owner-driven correction passes. Stage B is three things other tracks owe. **Stage C closed 2026-08-17** — twelve rows from an owner brief, eleven landed and one withdrawn, two of them reversing a standing ADR. **Stage D closed 2026-08-18** — one owner reading of the running block, four rows, all frontend: the time-saved foot was claiming a four-week window on a three-day record, and the same tile had no unit ceiling, so the span now ramps and the figure climbs minutes to hours to days (ADR 0233); each metric opens its own view of the block, day to year, with a grain offered only once the ledger reaches three buckets of it (ADR 0235); the week starts on Monday everywhere. **Stage E closed 2026-08-18** — the same evening, five rows from reading Stage D's views against 447 real dictations: turnaround is five bands rather than a 25 ms histogram with eleven empty columns, and under them a list of which model and vendor the wait belongs to; the record now stores the language it was counted as, because the naming call answered for 74 of 75 runs and nothing wrote the answer down, so every ledger rebuild re-measured with the offline detector and dropped the short ones; the decimal point stopped touching the digit before it; the view dots leave the screen inside a metric instead of merely going inert (ADR 0236). **The finding worth knowing outside this track**: five of Stage D's defects were green in Vitest and visible only on a rendered page, one of them a chart drawn at 16 px because `.ws-win svg` is an unlayered presentation hint that beats the stylesheet — the trap the calendar's own file documents and solves with an inline style — and Stage E is the same rule again with the REAL config, ledger and history behind a stubbed `__TAURI_INTERNALS__`, which is the only way one recogniser showing up under two vendors was ever going to be visible. **Stage F closed 2026-08-19** — privacy work that came out of Stage E's cause list: that list reads `history.json`, the index holds a thousand records, and at 196 dictations a day that is about five days — which was also, until this stage, the lifetime of every Markdown transcript the reader owns, because the retention sweep deleted the file with the entry. The sweep no longer does; deleting a record, clearing the history and a new purge still do. The archive became the fourth collection on Privacy & Data with a count, a size and a `Delete now` that walks the store's own layout and leaves a file the reader put in that folder alone (ADR 0237). **What it does not fix, and the owner was told before it was built**: the cause list still reaches five days, because the ceiling is unchanged and the index is what it reads. **Stage G closed 2026-08-19** — the owner declined that repair and asked for the cause instead: *at a thousand records this stops making sense, so optimise it*, widened to *every function gets only what it really needs*. Five rows. The index is read when a record-writing event says so rather than every five seconds — the poll was moving 14.1 MB a minute of an index that changes 196 times a day, and it is the worse of the three findings; a list row became a `TranscriptionHistorySummary` with the transcripts cut to 160 characters and the whole record fetched by id, dropping sixteen fields no screen reads; the write is compact and lands through a rename; and the turnaround causes moved into the activity ledger as a bounded per-`provider/model` histogram, so the one reading on Home that was not all-time now is. Measured on the reader's real index: 2,453 bytes a row became 1,113, a 54.6% cut. `HISTORY_CEILING` went 1,000 → 5,000 on a release-build measurement (24.9 ms to serialise and write 5,000 records against a 1,210 ms median turnaround), with the note that past it the answer is an append-only journal (ADR 0240). **What it does not fix**: every term is still O(records) per dictation. **A second finding worth knowing outside this track**: the brief's premise was half wrong — the index is parsed once per process, not per dictation — and checking that is what located the cost on the write and read sides instead of the parse. **Stage H closed 2026-08-19** — the owner read Stage G's answer and rejected the UNIT rather than the number — five thousand dictations is not a unit that means anything, and the thing to bound is the storage rather than the file count. Five rows. The index is an append-only journal — one line per operation, `history.jsonl` — so a dictation's write costs the same at every index size: measured on a release build, 0.006 ms to append at 10,000 records against 38.3 ms to rewrite the file, which is the cost `HISTORY_CEILING` existed to bound. The ceiling is therefore deleted rather than raised a third time, and `history_limit` with it, out of the config, the IPC contract, the export document and the prune; what governs the index is `history_retention_days`, in months, which at 217 dictations a day was being overridden by a factor of sixteen. Both collections get a byte budget instead — 5 GB warning, 10 GB ceiling, independently, oldest first, enforced at startup — and the transcript archive gains a lifetime it had never had since ADR 0237 gave it its own. To make a byte ceiling one the layout can reach, the archive shards to `YYYY/MM/DD/` and `transcript_store_status` stopped stat-ing every file: a sidecar records a tally per day shard beside that shard's directory stamp, so only shards that moved are recounted and a reader deleting files by hand is noticed (ADR 0241, ADR 0242). **The finding worth knowing outside this track**: Stage G recorded the owner's `history_retention_days` as 7 and concluded retention was the binding rule; the live config said 365 and the conclusion inverted — **a number nobody read off the file it belongs to survived four commits and one ADR**. **Stage I closed 2026-08-19** — the owner opened it with a procedure and a constraint: verify every claim this page makes against the file before believing it, sort the raised items into defect / decision / not repairable / conditional, and then the constraint itself: every metric on Home must be able to exist indefinitely. Verifying found **four of the eight raised items misdescribed the code they pointed at**, and measuring the file found two defects nobody had raised: `activity.json` was written pretty-printed and non-atomically on every dictation — 21,326 bytes on disk against 5,634 of content, and it is the one file in a backup that cannot be rebuilt — and the 64-key cause cap dropped runs silently, making the display's own claim that the rows sum false. Seven rows. The ledger's readings become **fixed-size mergeable accumulators per period**, stored in three disjoint tiers — days rolling, months never pruned, one opaque row for the prehistory — so a pruned day keeps its shape and not just its figures; before this the *Years* view could hold three buckets at any installation age. Turnaround and languages gain per-period accumulators, the turnaround gains a second cut by mode, `Not named` splits into the two populations it always was, every field records the day it started being measured, and the file is written minified through a rename. **The finding worth knowing outside this track**: an `Accepted` ADR is not a closed door — Stage I overruled ADR 0176's day-row bound, on the owner's own instruction that fixed decisions are not fixed when something better exists (ADR 0243). **Stage J closed 2026-08-19** — opened within the hour Stage I closed, by the owner reading the screen it had just shipped and reporting that `Never asked` only makes sense for the one developer with accumulated local data. Checking it found the row did not measure what its own ADR claimed — it measured how far back the SEED could reach — and found a second defect nobody had raised: the facts list mixed a lifetime counter with the tiered ones and **summed to 653 against 586 dictations**. Five rows. The owner's answer was wider than the two defects: delete the accumulated local store, and build no migration and no second system that holds on to legacy local data in a developer build. So the ledger's opaque `retired` tier goes with its stamp and its schema migrations, the duplicate lifetime language counter goes, `Never asked` goes, a share is stated against the runs it was measured over, and the two turnaround cuts say in their headings that they are two cuts of the same runs (ADR 0244). **The finding worth knowing outside this track**: two cases that had been REWRITTEN rather than deleted when their behaviour was removed caught a defect in the removal itself — a horizon rewritten to `ledgerFirstDay` would have drawn a year of empty buckets, because that one prefers `started_on` and the day tier does not reach it. **And the licence expires**: the no-legacy rule holds only while no release exists, which ADR 0244 states rather than leaving to be inferred. **Stage K closed 2026-08-22, and was verified and finished 2026-08-23** — opened the day after Stage J by the owner reading the turnaround detail it had just corrected and getting five questions off one screen, none of which the note under the lists answered. The ambiguity was not in the copy: `turnaround_causes` and `mode_causes` were fed **the same `turnaround_ms`**, so `Which model heard it` said 0.9 s and `What the mode cost` said 0.9 s over 129 of the same 138 runs — one measurement filed under two keys and drawn under two headings that each promised a stage the record had never held. No arithmetic over the store could recover the split, so the owner chose to measure it rather than rename the headings: `heard_ms` is stamped the instant the provider returns and BEFORE the staleness check, the rewriting is the remainder, and the pair travels as one `TurnaroundFacts` because two adjacent `Option<u64>` in a long signature are told apart only by position. The two lists became one table with named columns and a `by model` / `by mode` toggle, every `ws-metric-note` paragraph went, and each of the four details now opens with its reading in one line (ADR 0247). Nothing is backfilled — a seed that guessed at the split would be the plausible wrong number the track exists against. **K4, the next day**: the first real dictations put the table into the state neither shipped shape covered, and it printed `heard in 0.5 s` beside `in total 0.9 s` — the first over five runs, the second over 147, on one line with only a hover tooltip between them. A row now carries the run count each figure rests on (`4/137`, `1/6`, `0/2`) and the heading names the column that is thin, both deleting themselves when the split covers the runs (ADR 0248). **Two findings worth knowing outside this track**: every case for that table used a fixture whose split covered all of a row's runs or none, so **a range with two ends has three states and the fixtures covered two**; and Stage K's own record and commit message both quote 1043 Rust cases against a tree that ran 1045 — read the figure off `cargo test -- --list`, not off the tail of a run. **Stage B is what is left and it is three things other tracks owe** | [`tracks/home-activity.md`](tracks/home-activity.md) | the same file — it is both |
...
replaced. **A tenth was added and closed on 2026-08-23** — the word `heard`
named a boundary one stage later than it promised, and ADR 0249 moved it
(item 11).
...
0189–0196 for Stage C, and holds **0233**, **0235**, **0236**, **0237**,
**0240**, **0241** and **0242** for Stages D to H; 0234 and then 0238 were both
lost to the insert-delivery track mid-session and renumbered.
...
- Read an old file only where reading it is trivial. Where it is not, drop it
  and let the store rebuild itself. The journal of ADR 0241 is the worked
  example: `history.json` is read once, converted and deleted, because the parse
  for it was already in the file and reading it cost six lines.
