...
| **Speech** | 2026-08-11 | **Stage A closed, Stage B running, Stage D opened**; 21 of ~31 steps done. **B13, B14 and B14a all landed 2026-08-17 (ADR 0207, ADR 0208, ADR 0209)**: every model is per profile, and so is the whole connection — a stored account object carrying the vendor, the endpoint, the plan and the credential's scope, which a profile points at per job. **B14a was the owner operating that row within the hour** and finding four surface faults, of which the load-bearing one was a status keyed by vendor reporting one account's key under another. **Two steps opened out of it**: B14b, a removed account still leaves its credential in the OS store; and **B15, which is a restructure rather than a fix** — the word *Account* names a credential while the reader hears a bundle, and the config already supports a different lane per task while three UI locks forbid it. B15 is blocked on one owner decision about the model axis | [`tracks/speech-track-plan.md`](tracks/speech-track-plan.md) | the plan is where a session starts, except for B15, which has its own brief: [`tracks/speech-track-b15-kickoff.md`](tracks/speech-track-b15-kickoff.md). [`tracks/speech-track.md`](tracks/speech-track.md) is stage one's record: background, not a brief |
