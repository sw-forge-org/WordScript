...
| **Speech** | 2026-08-11 | **Stage A closed, Stage B running, Stage D opened**; 20 of ~29 steps done, and the workspace's own strip now names whichever connection it is on. **B13 and B14 both landed 2026-08-17 (ADR 0207, ADR 0208)**: every model is per profile, and so is the whole connection — a stored account object carrying the vendor, the endpoint, the plan and the credential's scope, which a profile points at per job | [`tracks/speech-track-plan.md`](tracks/speech-track-plan.md) | the same file — the plan is where a session starts. [`tracks/speech-track.md`](tracks/speech-track.md) is stage one's record: background, not a brief |
