...
| **Speech** | 2026-08-11 | **Stage A closed, Stage B running, Stage D opened**; 18 of ~29 steps done, and the workspace's own strip now names whichever connection it is on. **B13 landed 2026-08-17 (ADR 0207) and opened B14**: every model is per profile now, the server's URL and the credential are not | [`tracks/speech-track-plan.md`](tracks/speech-track-plan.md) | the same file — the plan is where a session starts. **For B14 specifically: [`tracks/connection-per-profile-kickoff.md`](tracks/connection-per-profile-kickoff.md).** [`tracks/speech-track.md`](tracks/speech-track.md) is stage one's record: background, not a brief |
