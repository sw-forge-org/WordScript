...
### B14. A profile carries its whole connection, not only which vendor it is (ADR 0094, ADR 0207) — **done 2026-08-17, ADR 0208**

**Closed the day after it was filed, and the shape is B**: a connection is a
stored object — vendor, endpoint, model id, plan and the credential's scope —
and a profile points at one per job. The decision was the owner's and the first
recommendation put to them was **A with a machine-wide fallback**; two
corrections moved it. The seeded profile set is being reworked, so *a fresh
install has six profiles* was not an argument that could carry a fallback, and
a profile is meant to carry connections rather than only a writing style. What
decided it in the end was the owner's own counter-proposal — *several keys,
labelled, visible under each profile in the Models tab* — which is B stated from
the surface.

What landed: `AppConfig::connections`, `JobProvider` carrying the connection
beside the derived vendor, `credential_store`'s scope moved from the vendor id
to the account's with a `rekey` that **moves** rather than copies, the plan and
the self-hosted endpoint lifted onto the account that owns them, and one new row
on the connection card — `Account`, with `Rename`, `New` and `Remove` — between
the vendor and its credential. **+9 Rust (936) and +4 frontend (840)**, the four
proven by five mutations, one of which ran green against the wrong save door
first. `port:diff`: `models` `26 | 248 | 20` → `28 | 248 | 20`, the two
...
[`../archive/speech-track-b14-kickoff.md`](../archive/speech-track-b14-kickoff.md)**,
which is orientation; this section is the brief. (The kick-off is spent and
archived — the step closed on 2026-08-17.)
