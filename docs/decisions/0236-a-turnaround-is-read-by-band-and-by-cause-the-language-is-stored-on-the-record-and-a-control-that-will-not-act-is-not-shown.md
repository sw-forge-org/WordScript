...
**1. The turnaround chart asked no question.** The report was that Turnaround is
extremely hard to read: the reader cannot tell what the view is claiming, and a
column reading *4.5 s to 4.9 s, 3 dictations* beside another saying the same
thing with 0 dictations leaves them with nothing to do with either.
...
number.** The objection was that Languages cannot honestly say that — a
deterministic language detection and a naming model had both been built in, so
how do 91 runs end up too short to name?

The owner is right that both instruments exist, and the count is real: 447 dictations,
...
**3. A decimal point merged into the digit before it.** Reported on Time saved —
a pixel of padding missing between the `3` and the point — and reported against
Turnaround, where the same component already had it right.
...
**4. The dots kept offering a view the block would not go to.** First: from
inside a zoomed metric view, switching back to the calendar view should be
prevented.

They were disabled. That was half an answer: the two dots still sit under every
enlarged metric view, still implying the calendar is one press away —
mechanically switched off, visually still there.
...
same round — it would make sense for Turnaround to show which AI model was
mainly responsible for the wait, since that is what a turnaround is about — and
its first build printed the vendor beside the model with nothing between them.
`Whisper Large V3 Turbo OpenAI` left the reader asking what the second name is:
the model's owner, the profile, or the provider it ran through.
