# GUI port relay — the closed leg records

Legs 1 through 8, in the order they ran. Each record is the account its leg
wrote when it closed: what landed, what it found, and what it deliberately
did not do.

The relay keeps the four most recent records, because a leg's starting state
is the record of the one before it; everything older lives here. The one-line
summary of every leg, including these, stays in the relay's leg-log table.

Relay: [`../tracks/gui-port-relay.md`](../tracks/gui-port-relay.md).
The briefs these legs ran against:
[`gui-port-relay-prompts.md`](gui-port-relay-prompts.md).
...
Leg 2b flips it, in this document and in `plans/settings-rework.md` §0.
...
document and in `plans/settings-rework.md` §0, and writes the Leg 3 prompt.
...
- **THE COMMUNICATION STYLE HAS NO SURFACE ANYWHERE, AND IT IS STILL RUNNING.**
  ...
  **So the port carried the absence faithfully and Leg 4c had nothing to wire.**
  ...
  that was never drawn. `plans/settings-rework.md` §11.4 required the card to
...
`plans/settings-rework.md` §0. **Leg 2 is closed.**
...
Corrected in `plans/settings-rework.md` §4.2 (which also still said five views;
...
`plans/settings-rework.md`, this file and `CHANGELOG.md`. No component, no route,
